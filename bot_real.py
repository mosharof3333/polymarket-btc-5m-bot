...
#   PK     → your MetaMask private key (64 hex chars, WITHOUT 0x prefix)
#   FUNDER → your Polymarket proxy wallet address (visible in Polymarket UI)
...
_FUNDER = os.environ.get("FUNDER", "")

if not _PK or not _FUNDER:
    raise SystemExit(
        "❌  Missing credentials.\n"
        "    Set PK and FUNDER as environment variables in Railway before running."
    )
...
_clob  = ClobClient(
    ...
    signature_type = 2,         # 2 = POLY_GNOSIS_SAFE (Polymarket proxy wallet)
    funder         = _FUNDER,
)
_creds = _clob.create_or_derive_api_creds()
_clob.set_api_creds(_creds)
print(f"✅ Connected | Proxy wallet: {_FUNDER}\n")

# ─────────────────────────────────────────────────────────────────────────────

STATE_FILE           = "bot_state_real.json"   # separate file, won't mix with paper bot
TRIGGER_CHEAP         = 0.30
...
async def _place_order(order_args: MarketOrderArgs, order_type=OrderType.FOK):
    """Run blocking py_clob_client call in thread executor."""
    loop = asyncio.get_event_loop()
    signed = await loop.run_in_executor(
        None, partial(_clob.create_market_order, order_args)
    )
    resp = await loop.run_in_executor(
        None, partial(_clob.post_order, signed, order_type)
    )
    return resp
...
def settle_side_at_dollar(state, side, winner):
    """
    At hard expiry Polymarket auto-redeems tokens — no order needed.
    This just updates our capital tracking.
    """
...
async def buy_position(state, session, side, bet, ask, label=""):
    """Place a real market buy order on Polymarket CLOB, then update state."""
    token_id   = token_for(state, side)
    order_args = MarketOrderArgs(token_id=token_id, amount=bet, side=BUY)

    try:
        resp = await _place_order(order_args, OrderType.FOK)
        if not resp or resp.get("errorMsg"):
            print(f"⚠️  BUY order failed: {resp}")
            return False
    except Exception as e:
        print(f"⚠️  BUY order error: {e}")
        return False
...
async def sell_position(state, session, side, reason="TP"):
    """Place a real market sell order on Polymarket CLOB, then update state."""
    if side == state.cheap_side:
        shares = state.cheap_shares
        cost   = state.cheap_cost
    else:
    ...
    try:
        resp = await _place_order(order_args, OrderType.FOK)
        if not resp or resp.get("errorMsg"):
            print(f"⚠️  SELL order failed: {resp}")
    except Exception as e:
        print(f"⚠️  SELL order error: {e}")
...
def print_stats(state):
    ...
    print(f"📈 STATS ─────────────────────────────────────────")
    print(f"   First buy  (${FIRST_BET} @ 0.30):  "
...
async def main():
    state = load_state()
    print(f"🚀 BTC 5m Bot (REAL MONEY) | Capital {cap(state.capital)} | Phase: {state.phase}")
    ...
    print(f"   TP @ {TP} for both | Strong SL @ {SL_STRONG}")
    print_stats(state)
          ...
          try:
            ...
            # ── PHASE: watching ───────────────────────────────────────────
                ...
                if up_ask <= TRIGGER_CHEAP:
                    ...
                    bought = await buy_position(state, session, "up", FIRST_BET, up_ask)
                    if bought:
                    ...
                    bought = await buy_position(state, session, "down", FIRST_BET, dn_ask)
                    if bought:
                        state.phase = "first_active"
                        save_state(state)
            ...
            # ── PHASE: first_active ───────────────────────────────────────
                ...
                if cheap_ask >= TP:
                    ...
                    bought = await buy_position(state, session, state.strong_side, SECOND_BET, strong_ask)
                    if bought:
